...
# ============================================================
#  扩展模块：ecsspeed 三网/国际节点测速（基于 spiritLHLS/ecsspeed）
# ============================================================
...
ecs_install_speedtest() {
    ...
    if [ -f "${ECS_CLI_DIR}/speedtest" ] || [ -f "${ECS_CLI_DIR}/speedtest-go" ]; then
        return 0
    fi
    mkdir -p "${ECS_CLI_DIR}"
    cd "${ECS_WORK_DIR}" || return 1
    ...
    curl --fail -sL -m 15 -o speedtest.tgz "${url1}" 2>/dev/null || \
        curl --fail -sL -m 15 -o speedtest.tgz "${url2}" 2>/dev/null

    if [ -f "./speedtest.tgz" ]; then
        tar -zxf speedtest.tgz -C "${ECS_CLI_DIR}" && chmod 777 "${ECS_CLI_DIR}/speedtest" && rm -f speedtest.tgz
    else
        echo -e "${YELLOW}官方工具下载失败，尝试 speedtest-go...${ENDC}"
        local go_bit="$sys_bit"
        [ "$go_bit" = "aarch64" ] && go_bit="arm64"
        ...
        curl --fail -sL -m 20 -o speedtest.tar.gz "${url3}" 2>/dev/null || \
            curl --fail -sL -m 20 -o speedtest.tar.gz "${cdn_success_url}${url3}" 2>/dev/null
        if [ -f "./speedtest.tar.gz" ]; then
            tar -zxf speedtest.tar.gz -C "${ECS_CLI_DIR}" && chmod 777 "${ECS_CLI_DIR}/speedtest-go" && rm -f speedtest.tar.gz
        else
...
ecs_speed_test_single() {
    local server_id="$1" nodeName="$2"
    cd "${ECS_WORK_DIR}" || return
    if [ -f "${ECS_CLI_DIR}/speedtest" ]; then
        ...
        if [ $? -eq 0 ]; then
            local dl_speed; dl_speed=$(awk '/Download/{print $3" "$4}' "${ECS_CLI_DIR}/speedtest.log")
            local up_speed; up_speed=$(awk '/Upload/{print $3" "$4}' "${ECS_CLI_DIR}/speedtest.log")
            local latency; latency=$(grep -oP 'Idle Latency:\s+\K[\d\.]+' "${ECS_CLI_DIR}/speedtest.log")
            local pkt_loss; pkt_loss=$(awk -F': +' '/Packet Loss/{if($2=="Not available."){print "NULL"}else{print $2}}' "${ECS_CLI_DIR}/speedtest.log")
            [ -n "$dl_speed" ] || [ -n "$up_speed" ] || [ -n "$latency" ] && \
                echo -e "${nodeName}\t ${up_speed}\t ${dl_speed}\t ${latency}ms\t ${pkt_loss}"
        fi
        ...
        if [ $? -eq 0 ]; then
            local dl_speed; dl_speed=$(grep -oP 'Download: \K[\d\.]+' "${ECS_CLI_DIR}/speedtest.log")
            local up_speed; up_speed=$(grep -oP 'Upload: \K[\d\.]+' "${ECS_CLI_DIR}/speedtest.log")
            local latency; latency=$(grep -oP 'Latency: \K[\d\.]+' "${ECS_CLI_DIR}/speedtest.log")
            [ -n "$dl_speed" ] || [ -n "$up_speed" ] || [ -n "$latency" ] && \
...
ecs_test_list() {
    local list=("$@")
    for item in "${list[@]}"; do
        local id; id=$(echo "$item" | cut -d',' -f1)
        local name; name=$(echo "$item" | cut -d',' -f2)
        ecs_speed_test_single "$id" "$name"
    done
}
...
ecs_runtest() {
    ...
    case "$sel" in
    1)  # 三网就近
        command -v ping >/dev/null 2>&1 || { echo -e "${YELLOW}ping 不可用，跳过延迟排序${ENDC}"; sel=2; }
        echo -e "${CYAN}正在探测各运营商最近节点...${ENDC}"
        local CN_Unicom=();  CN_Unicom=($(ecs_get_nearest_data  "${SERVER_BASE_URL}/CN_Unicom.csv"))
        local CN_Telecom=(); CN_Telecom=($(ecs_get_nearest_data "${SERVER_BASE_URL}/CN_Telecom.csv"))
        local CN_Mobile=();  CN_Mobile=($(ecs_get_nearest_data  "${SERVER_BASE_URL}/CN_Mobile.csv"))
        ecs_temp_head
        ecs_test_list "${CN_Unicom[@]}"
        ecs_test_list "${CN_Telecom[@]}"
        ecs_test_list "${CN_Mobile[@]}"
        ;;
    2)  # 三网全测
        local CN_Unicom=();  CN_Unicom=($(ecs_get_data  "${SERVER_BASE_URL}/CN_Unicom.csv"))
        local CN_Telecom=(); CN_Telecom=($(ecs_get_data "${SERVER_BASE_URL}/CN_Telecom.csv"))
        local CN_Mobile=();  CN_Mobile=($(ecs_get_data  "${SERVER_BASE_URL}/CN_Mobile.csv"))
        ecs_temp_head
        ecs_test_list "${CN_Unicom[@]}"
        ecs_test_list "${CN_Telecom[@]}"
        ecs_test_list "${CN_Mobile[@]}"
        ;;
    3)  local slist=(); slist=($(ecs_get_data "${SERVER_BASE_URL}/CN_Unicom.csv")); ecs_temp_head; ecs_test_list "${slist[@]}" ;;
    4)  local slist=(); slist=($(ecs_get_data "${SERVER_BASE_URL}/CN_Telecom.csv")); ecs_temp_head; ecs_test_list "${slist[@]}" ;;
    5)  local slist=(); slist=($(ecs_get_data "${SERVER_BASE_URL}/CN_Mobile.csv")); ecs_temp_head; ecs_test_list "${slist[@]}" ;;
    6)  local slist=(); slist=($(ecs_get_data "${SERVER_BASE_URL}/HK.csv")); ecs_temp_head; ecs_test_list "${slist[@]}" ;;
    7)  local slist=(); slist=($(ecs_get_data "${SERVER_BASE_URL}/TW.csv")); ecs_temp_head; ecs_test_list "${slist[@]}" ;;
    8)  local slist=(); slist=($(ecs_get_data "${SERVER_BASE_URL}/JP.csv")); ecs_temp_head; ecs_test_list "${slist[@]}" ;;
    9)  local slist=(); slist=($(ecs_get_data "${SERVER_BASE_URL}/SG.csv")); ecs_temp_head; ecs_test_list "${slist[@]}" ;;
    *)  echo "已取消"; return ;;
    esac
    echo "——————————————————————————————————————————————————————————————————————————————"
}
...
run_ecsspeed_test() {
    ecs_check_cdn_file
    ecs_install_speedtest || { echo -e "${RED}speedtest 工具安装失败，无法执行三网测速${ENDC}"; return 1; }
    echo
    echo "  三网/国际 speedtest.net 节点测速（数据来源: spiritLHLS/ecsspeed）"
    echo "——————————————————————————————————————————————————————————————————————————————"
    echo -e "  ${GREEN}1.${ENDC} 三网测速（就近节点）     ${GREEN}6.${ENDC} 香港"
    echo -e "  ${GREEN}2.${ENDC} 三网测速（所有节点）     ${GREEN}7.${ENDC} 台湾"
    echo -e "  ${GREEN}3.${ENDC} 联通                     ${GREEN}8.${ENDC} 日本"
    echo -e "  ${GREEN}4.${ENDC} 电信                     ${GREEN}9.${ENDC} 新加坡"
    echo -e "  ${GREEN}5.${ENDC} 移动                     ${GREEN}0.${ENDC} 返回主菜单"
    echo "——————————————————————————————————————————————————————————————————————————————"
    local sel
    while true; do
        read -r -p "请选择: " sel
        [[ "$sel" =~ ^[0-9]$ ]] && break
        echo -e "${RED}无效输入${ENDC}"
    done
    [[ "$sel" == "0" ]] && return
    local ts; ts=$(date +%s)
    ecs_runtest "$sel"
    local te; te=$(date +%s)
    local elapsed=$(( te - ts ))
    echo " 总共花费: ${elapsed} 秒 | $(date)"
    rm -f "${ECS_CLI_DIR}/speedtest.log"
}
# ============================================================
#  扩展模块结束
# ============================================================
...
main_menu() {
    while true; do
        ...
        echo "12. 三网/国际 speedtest.net 测速"
        echo "0. 退出"
        echo
        read -r -p "请选择: " menu
        case "$menu" in
