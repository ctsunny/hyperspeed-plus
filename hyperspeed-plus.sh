...
show_nodes() {
    ...
    echo; echo "输入示例: 1,2,5"; echo "输入 all 表示全选"
}
...
select_nodes() {
    ...
    while true; do
        ...
        for token in "${TOKENS[@]}"; do
            ...
            token_num=$((10#$token))
            if (( token_num < 1 || token_num > ${#NODES[@]} )); then valid=0; break; fi
        done
        if (( valid == 0 )); then echo -e "${RED}输入无效，请重新输入${ENDC}"; continue; fi
        for token in "${TOKENS[@]}"; do
            token_num=$((10#$token))
            if ! array_contains "$token_num" "${SELECTED_IDS[@]}"; then SELECTED_IDS+=("$token_num"); fi
        done
...
run_test_plan() {
    ...
    for id in "${SELECTED_IDS[@]}"; do
        entry="${NODES[$((id-1))]}"; IFS='|' read -r group location isp extra dl ul <<< "$entry"
    ...
    [[ -n "$THREAD_FLAG" ]] && log_line "${CYAN}八线程${ENDC}" "八线程" || log_line "${CYAN}单线程${ENDC}" "单线程"
    while true; do
...
run_single_test() {
    ...
    entry="${NODES[$((id-1))]}"; IFS='|' read -r group location isp extra dl_b64 ul_b64 <<< "$entry"
    dl=$(decode_b64 "$dl_b64"); ul=$(decode_b64 "$ul_b64")
    node_name=$("$BINARY" -n "$location" 2>/dev/null); [ -n "$node_name" ] || node_name="$location"
    local cmd=("$BINARY" "$dl" "$ul")
    [ -n "$THREAD_FLAG" ] && cmd+=("$THREAD_FLAG")
...
run_test_plan() {
    ...
    log_line "CSV: ${CSV_FILE}" "CSV: ${CSV_FILE}"
...
list_csvs() {
    ...
    if [ ${#CSV_FILES[@]} -eq 0 ]; then echo -e "${YELLOW}暂无CSV日志${ENDC}"; return 1; fi
    echo; local i
...
view_latest_log() {
    mapfile -t LOG_FILES < <(find "$LOG_DIR" -maxdepth 1 -type f -name '*.log' | sort -r)
    if [ ${#LOG_FILES[@]} -eq 0 ]; then echo -e "${YELLOW}暂无日志${ENDC}"; return; fi
...
generate_summary_report() {
    ...
    END {
        ...
        print "样本总数: " total; print "测试轮数: " rounds;
        ...
        if (success > 0) {
            ...
            print ""; print "可用样本统计(仅双向正常):";
            printf "- 平均上传: %.2f Mbps 波动: %.2f\n", up_avg, up_sd;
            printf "- 平均下载: %.2f Mbps 波动: %.2f\n", down_avg, down_sd;
            printf "- 平均延迟: %.2f ms 波动: %.2f\n", lat_avg, lat_sd;
            printf "- 平均抖动: %.2f ms 波动: %.2f\n", jit_avg, jit_sd;
            printf "- 峰值上传: %.2f Mbps (%s)\n", best_up_val, best_up;
            printf "- 峰值下载: %.2f Mbps (%s)\n", best_down_val, best_down;
            printf "- 最低延迟: %.2f ms (%s)\n", best_lat_val, best_lat;
            printf "- 最低抖动: %.2f ms (%s)\n", best_jit_val, best_jit;
            if (down_avg >= 80 && lat_avg <= 160 && jit_avg <= 8) print "- 综合评价: 质量较好";
            else if (down_avg >= 40 && lat_avg <= 200) print "- 综合评价: 线路可用，高峰需继续观察";
            else print "- 综合评价: 线路存在短板，建议延长压测";
            if (down_sd > (down_avg * 0.25)) print "- 下载波动偏大，带宽稳定性一般";
            if (lat_sd > 20) print "- 延迟波动明显，实时业务可能受影响";
            if (jit_avg > 10) print "- 平均抖动偏高，建议观察拥塞情况";
        } else { print ""; print "可用样本统计: 暂无双向正常样本"; }
        print ""; print "分组画像:";
        for (g in group_total) {
            rate=(group_ok[g]+0)/group_total[g]*100;
            printf "- %s: 双向可用率 %.2f%% (%d/%d)", g, rate, group_ok[g]+0, group_total[g];
            if ((group_dual_ok[g]+0) > 0) printf "，上传 %.2f 下载 %.2f 延迟 %.2f 抖动 %.2f", group_up_sum[g]/group_dual_ok[g], group_down_sum[g]/group_dual_ok[g], group_lat_sum[g]/group_dual_ok[g], group_jit_sum[g]/group_dual_ok[g];
            printf "\n";
        }
        print ""; print "节点画像:";
        for (n in node_total) {
            ...
            if ((node_dual_ok[n]+0) > 0) printf "，上传 %.2f 下载 %.2f 延迟 %.2f 抖动 %.2f", node_up_sum[n]/node_dual_ok[n], node_down_sum[n]/node_dual_ok[n], node_lat_sum[n]/node_dual_ok[n], node_jit_sum[n]/node_dual_ok[n];
            printf "\n";
        }
        print "——————————————————————————————————————————";
    }' "$csv_file" > "$out_file"
...
generate_speed_svg() {
    ...
    END {
        ...
        print "<text x=\"40\" y=\"28\" fill=\"#e5e7eb\" font-size=\"20\" font-family=\"Arial\">速度曲线（按轮次）</text>";
        ...
        for(i=0;i<=4;i++){y=T+pH-(pH*i/4);v=maxV*i/4;print "<line x1=\"" L "\" y1=\"" y "\" x2=\"" L+pW "\" y2=\"" y "\" stroke=\"#1f2937\" stroke-width=\"1\"/>";printf "<text x=\"4\" y=\"%.0f\" fill=\"#94a3b8\" font-size=\"11\" font-family=\"Arial\">%.0f</text>\n",y+4,v;}
        if(n==0){print "<text x=\"100\" y=\"240\" fill=\"#fbbf24\" font-size=\"18\" font-family=\"Arial\">暂无数据</text></svg>"; exit;}
        step=(n==1)?0:pW/(n-1);
        uP=""; dP="";
        for(i=1;i<=n;i++){x=L+(i-1)*step;yu=T+pH-(up[i]/maxV*pH);yd=T+pH-(down[i]/maxV*pH);uP=uP sprintf("%.1f,%.1f ",x,yu);dP=dP sprintf("%.1f,%.1f ",x,yd);print "<circle cx=\"" x "\" cy=\"" yu "\" r=\"3\" fill=\"#38bdf8\"/>";print "<circle cx=\"" x "\" cy=\"" yd "\" r=\"3\" fill=\"#22c55e\"/>";printf "<text x=\"%.1f\" y=\"%d\" fill=\"#94a3b8\" font-size=\"10\" font-family=\"Arial\" text-anchor=\"middle\">%s</text>\n",x,T+pH+18,label[i];}
        ...
        print "<rect x=\"1080\" y=\"14\" width=\"12\" height=\"12\" fill=\"#38bdf8\"/><text x=\"1098\" y=\"25\" fill=\"#e5e7eb\" font-size=\"13\" font-family=\"Arial\">上传</text>";
        print "<rect x=\"1140\" y=\"14\" width=\"12\" height=\"12\" fill=\"#22c55e\"/><text x=\"1158\" y=\"25\" fill=\"#e5e7eb\" font-size=\"13\" font-family=\"Arial\">下载</text>";
        print "</svg>";
    }' "$data_file" > "$out_svg"
}
...
generate_latency_svg() {
    ...
    END {
        ...
        print "<text x=\"40\" y=\"28\" fill=\"#e5e7eb\" font-size=\"20\" font-family=\"Arial\">延迟曲线（按轮次）</text>";
        print "<line x1=\"" L "\" y1=\"" T "\" x2=\"" L "\" y2=\"" T+pH "\" stroke=\"#94a3b8\" stroke-width=\"1\"/>";
        print "<line x1=\"" L "\" y1=\"" T+pH "\" x2=\"" L+pW "\" y2=\"" T+pH "\" stroke=\"#94a3b8\" stroke-width=\"1\"/>";
        for(i=0;i<=4;i++){y=T+pH-(pH*i/4);v=maxV*i/4;print "<line x1=\"" L "\" y1=\"" y "\" x2=\"" L+pW "\" y2=\"" y "\" stroke=\"#1f2937\" stroke-width=\"1\"/>";printf "<text x=\"4\" y=\"%.0f\" fill=\"#94a3b8\" font-size=\"11\" font-family=\"Arial\">%.0f</text>\n",y+4,v;}
        if(n==0){print "<text x=\"100\" y=\"240\" fill=\"#fbbf24\" font-size=\"18\" font-family=\"Arial\">暂无数据</text></svg>"; exit;}
        step=(n==1)?0:pW/(n-1);
        lP=""; jP="";
        for(i=1;i<=n;i++){x=L+(i-1)*step;yl=T+pH-(lat[i]/maxV*pH);yj=T+pH-(jit[i]/maxV*pH);lP=lP sprintf("%.1f,%.1f ",x,yl);jP=jP sprintf("%.1f,%.1f ",x,yj);print "<circle cx=\"" x "\" cy=\"" yl "\" r=\"3\" fill=\"#f59e0b\"/>";print "<circle cx=\"" x "\" cy=\"" yj "\" r=\"3\" fill=\"#a855f7\"/>";printf "<text x=\"%.1f\" y=\"%d\" fill=\"#94a3b8\" font-size=\"10\" font-family=\"Arial\" text-anchor=\"middle\">%s</text>\n",x,T+pH+18,label[i];}
        ...
        print "<rect x=\"1080\" y=\"14\" width=\"12\" height=\"12\" fill=\"#f59e0b\"/><text x=\"1098\" y=\"25\" fill=\"#e5e7eb\" font-size=\"13\" font-family=\"Arial\">延迟</text>";
        print "<rect x=\"1140\" y=\"14\" width=\"12\" height=\"12\" fill=\"#a855f7\"/><text x=\"1158\" y=\"25\" fill=\"#e5e7eb\" font-size=\"13\" font-family=\"Arial\">抖动</text>";
        print "</svg>";
    }' "$data_file" > "$out_svg"
}
...
<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HyperSpeed Plus 报告</title>
